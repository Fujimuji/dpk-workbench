...
export function readLavaOrbs(
  ...
  if (!isArrayValue(parsedLocations)) {
    throw new ParseError('invalid_syntax', `Global.c_checkpointLavaLocations[${levelKey}] must be Null or an Array.`);
  }
  ...
  const sizeSlots =
    parsedSizes === null || parsedSizes === false
      ...
      : isArrayValue(parsedSizes)
        ? parsedSizes
        : (() => {
            throw new ParseError('invalid_syntax', `Global.c_checkpointLavaSizes[${levelKey}] must be Null or an Array.`);
          })();
  ...
  for (let index = 0; index < checkpointConfigs.length; index += 1) {
    ...
    if (locationSlot === null || locationSlot === false || locationSlot === undefined) {
      checkpointConfigs[index].lava = null;
      continue;
    }

    const positions = normalizeVectorList(locationSlot, `Global.c_checkpointLavaLocations[${levelKey}][${index}]`);
    const sizeValues =
      sizeSlots[index] === undefined || sizeSlots[index] === null || sizeSlots[index] === false
        ? []
        : normalizeNumberList(sizeSlots[index], `Global.c_checkpointLavaSizes[${levelKey}][${index}]`);

    checkpointConfigs[index].lava = positions.map((position, orbIndex) => ({
      position,
